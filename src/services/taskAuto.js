  ...
  getTasks: (page = 1, itemsPerPage = 10) => {
    return new Promise((resolve, reject) => {
      api()
        ...
        .then(({ status, data }) => {
          if (status === 200 || status === 200) resolve(data);
  ...
  getTaskById: (id) => {
    return new Promise((resolve, reject) => {
        api()
            .get(`/assign-tasks/${id}`)
            .then(({ status, data }) => {
                if (status === 200 || status === 200) resolve(data.data);
                throw new Error(`Failed with status: ${status}`);
            })
            .catch(error => reject(error));
    });
  ...
  deleteTask: (id, data) => {
    return new Promise((resolve, reject) => {
        api()
            ...
            .then(({ status, data }) => {
                if (status === 201 || status === 200) resolve(data.data);
                throw new Error(`Failed with status: ${status}`);
            })
            .catch(error => reject(error));
    });
