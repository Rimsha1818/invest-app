...
const CommentSystem = ({ id, form_id, status, comment_status }) => {
  ...
  useEffect(() => {
    const getComments = async () => {
      setLoading(true);
      try {
        const response = await commentService.getCommentsNonForm(id, form_id);
  ...
  const handleAddComment = async () => {
    form.validateFields().then(async (values) => {
      if (newComment || attachmentList.length > 0) {
        ...
        try {
          const response = await commentService.postCommentsNonForm(formData);
          setLoading(false);
          setFileList([]);
          setNewComment('');
          setAttachmentList([]);
