...
const ServiceDesk = () => {
  ...
  const handleDateChange = (date, dateString) => {
    console.log('date');       // Moment object
    console.log(date);       // Moment object
    console.log('dateString'); // Formatted date (string)
    console.log(dateString); // Formatted date (string)

    setSelectedDate(dateString); // store formatted string like "25-04-2025"
  };
  ...
  const onSearch = async (values) => {
    if (selectedDate) {
      ...
      const formattedDate = moment(selectedDate, 'DD-MM-YYYY').isValid()
        ? moment(selectedDate, 'DD-MM-YYYY').format('DD-MM-YYYY')
        : null;

      values.task_approval_at = formattedDate;  // properly format the date
    } else {
      values.task_approval_at = null;  // If no date, set to null
    }
    console.log('values')
    console.log(values)
    console.log('values')
    ...
    setLoading(true);
    try {
      const response = await formService.search(selectedForm, values);
  ...
  const handleSortChange = (pagination, filters, sorter) => {
    ...
    serviceDesk(selectedForm, page, itemsPerPage, sorter.field, sorter.order === "ascend" ? "asc" : "desc");
  };
  ...
  const ActionsDropdown = ({ record }) => {
    ...
    const handleComment = async (data) => {
      try {
        const response = await commentService.updateCommentStatus(data);
        if (response.success) {
    ...
    const menu = (
      <Menu onClick={handleMenuClick}>
        <Menu.Item key="1" icon={<EyeOutlined />}>
          {selectedForm === 2 && (
            <Link to={`/scrf/details/${record.id}`}>View</Link>
          )}
          {selectedForm === 3 && (
            <Link to={`/deployment/details/${record.id}`}>View</Link>
          )}
          {selectedForm === 4 && (
            <Link to={`/crf/details/${record.id}`}>View</Link>
          )}

          {selectedForm === 5 && (
            <Link to={`/mobile-requisition/details/${record.id}`}>View</Link>
          )}
          {selectedForm === 6 && (
            <Link to={`/master-data-management-form/details/${record.id}`}>
              View
            </Link>
          )}
          {selectedForm === 7 && (
            <Link to={`/sap-access-form/details/${record.id}`}>
              View
            </Link>
          )}
        </Menu.Item>
  ...
  const serviceDesk = async (
    ...
    try {
      setLoading(true);
      const response = await formService.serviceDesk(
        form,
        page,
        itemsPerPage,
        sortBy,
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (savedForm) {
        ...
        savedForm = selectedForm;
      }
      try {
        setLoading(true);
        const res = await serviceDesk(savedForm);
        // setData(res.data);
        setLoading(false);
      } catch (error) {
        console.error("Error fetching data:", error);
  ...
  const columns = [
    ...
    {
      title: "Start Date",
      dataIndex: ["task_assigned", "expected_start_time"],
      key: "task_assigned",
      // width: 200,
      sorter: true,
    },
    {
      title: "End Date",
      dataIndex: ["task_assigned", "expected_end_time"],
      key: "task_assigned",
      // width: 200,
      sorter: true,
    },
 ...
 const [visibleColumns, setVisibleColumns] = useState(() =>
  ...
  const handleColumnToggle = (key) => {
    ...
    useEffect(() => {
    try {
      const savedVisibleColumns = localStorage.getItem('visibleColumnsServiceDesk');
              ...
              <Card className="pt-6" style={{ paddingBottom: "0 !important" }}>
                <Row gutter={[12, 12]}>
                  ...
                  <Col lg={4} md={5} sm={6} xs={12}>
                    <Form.Item name="task_status">
                      <Select
                        ...
                        allowClear
                      >
                      <Select.Option key="no-status" value={null}>
                        No Status
                      </Select.Option>
                        {taskStatus &&
                          taskStatus.map((task) => (
                  ...
                  <Col lg={4} md={5} sm={6} xs={12}>
                    <Form.Item
                      name="manager_ids"
                      label="Select Assigned Managers"
                    >
                  ...
                  <Col lg={4} md={5} sm={6} xs={12}>
                    <Form.Item
                      label="Task Approved At"
                      name='task_approval_at'
                      valuePropName="value" 
                    >
                      <DatePicker
                        format={dateFormat}
                  ...
                  <Col lg={4} md={5} sm={6} xs={12}  className="d-flex align-items-center">
                    <p>&nbsp;</p>
                    <Button
                      block
                      className="btn-blue"
