...
const Mdm = () => {
  ...
  const [formId, setFormId] = useState(6);
  ...
  const onSearch = async (values) => {
    ...
    setMdmLoading(true);
    try {
      const response = await mdmService.searchMdm(values);
      //console.log(response)
  ...
  const getMdm = async (
    ...
    setMdmLoading(true);
    try {
      const response = await mdmService.getMdm(
        page,
        itemsPerPage,
        sortBy,
        sortOrder
  ...
  const handleDelete = (record) => {
    confirm({
      ...
      onOk: async () => {
        try {
          const response = await mdmService.deleteMdm(record.id);
          if (response.success) {
            notification.success({
              message: 'Deleted Successfully',
              description: response.message,
            });
            await getMdm(currentPage);
          }
        } catch (error) {
          notification.error({
            message: 'Error',
      ...
      <Row gutter={[24, 24]}>
        <Col span={24}>
          <Header
            ...
            title="Master Data Management Form"
            right={
              <>
                ...
                <Link to={`/master-data-management-form/add/`}>
                  <Button className="btn-blue" type="primary">
                    Add Master Data Management Form
                  </Button>
                </Link>
              </>
            }
