...
const Mdm = () => {
  ...
  const onSearch = async (values) => {
    ...
    setMdmLoading(true);
    try {
      const response = await deploymentService.searchDeployment(values);
      //console.log(response)
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: ['reference_form_id', 'name'],
    ...
    //   dataIndex: ['reference_detail', 'request_title'],
    //   // dataIndex: 'request_specs',
    //   key: 'request_specs',
    //   width: 180,
    //   sorter: true,
  ...
  const getDeployment = async (
    ...
    setMdmLoading(true);
    try {
      const response = await deploymentService.getDeployment(
        page,
        itemsPerPage,
        sortBy,
        sortOrder
  ...
  const handleDelete = (record) => {
    confirm({
      ...
      onOk: async () => {
        try {
          const response = await deploymentService.deleteDeployment(record.id);
          if (response.success) {
            notification.success({
              message: 'Deleted Successfully',
              description: response.message,
            });
            await getDeployment(currentPage);
          }
        } catch (error) {
          notification.error({
            message: 'Error',
