...
const Designations = () => {
  ...
  const handleDelete = (record) => {
    Modal.confirm({
      ...
      content: `Are you sure you want to delete the designation: ${record.name}?`,
      onOk: async () => {
        setLoading(true);
        try {
          const response = await designationService.deleteDesignation(record.id);
          if (response.success) {
            notification.success({
              message: "Designation Deleted",
              description: response.message,
            });
            getDesignations(currentPage);
          }
        } catch (error) {
          notification.error({
            message: "Error",
  ...
  const getDesignations = async (page = 1) => {
    setLoading(true);

    await designationService.getDesignations(page).then((response) => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      if (editing) {
        ...
        const response = await designationService.postDesignation(values);
    ...
    <DefaultLayout>
      ...
      <Card>
        <Table
          ...
          pagination={{
            ...
            onChange: (page, pageSize) => getDesignations(page),
          }}
          loading={loading}
        />
      </Card>
      ...
      >
        ...
        >
          ...
          >
            <Input size="large" placeholder="Type Designation Name" />
          </Form.Item>
